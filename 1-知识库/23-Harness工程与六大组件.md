...
- **一句话**：三个范式是**嵌套不是替代**（Prompt ⊂ Context ⊂ Harness）——**变量是 Harness，不是 Model**。
- **三要点**：①边际产出已转移：调 Prompt 三个月可能 +20%，Context 工程 +30~50%，搭对 Harness **两周 +47pp**；②裸 LLM 四大硬伤（无工作环境/不能执行代码/知识过时/无记忆）**↔ Harness 六大组件一一补上**；③**Hooks 是安全网**：pre / post / error / approval 四类，能在**执行前**拦截错误决策。
- **追问**：Harness 的六大组件分别解决什么问题？（→ [O-44](../2-面试题库/开放题/23-Harness工程与六大组件.md)）
